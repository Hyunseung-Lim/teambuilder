...
export const feedbackPrompt = (
  ...
  return `${profileContext}${memoryContext}${sharedMentalModelContext}Provide constructive feedback to ${target}.

**Context:** "${context}"

**Feedback Taxonomy Guidelines:**
Choose appropriate feedback approaches based on the situation:
...
**Task:** Generate constructive feedback in Korean using appropriate approaches.

JSON format:
{
  "target": "${target}",
  "feedbackType": "Primary approach used",
  "comment": "Detailed feedback in Korean"
}
`;
};
...
${profileContext}${memoryContext}${sharedMentalModelContext}`;
  ...
  const mainPrompt = `## User Request (Priority):
"${requestContext.originalMessage}" - ${requestContext.requesterName}

## Context:
**Team:** ${teamContext.teamName} | **Topic:** ${teamContext.topic}

**Members:** ${teamMembersInfo}
**Ideas:** ${ideasInfo}
**Recent Activity:** ${recentActivity}

## Task:
Determine optimal feedback approach prioritizing user intent:

1. **Target**: Select team member (use "나" for users, agentId for AI)
2. **Type**: specific_idea | strategy_improvement | team_dynamics  
3. **Message**: Constructive feedback in Korean reflecting user request
4. **Reasoning**: How user request was addressed
...
Respond in the following JSON format:
{
  ...
  "feedbackType": "specific_idea" | "strategy_improvement" | "team_dynamics",
  "targetIdea": {
...
1. Speak with respect for the other person’s expertise and views.
2. Give specific, practical feedback.
3. Ask questions to understand their ideas more deeply.
4. Reply to their latest message, drawing on your memory and past experience.
5. Keep it concise—around 200 characters.
...
// Individual agent persona summary generation prompt
export const generateAgentPersonaSummaryPrompt = (
  ...
  const agentContext = `You are an AI assistant specialized in creating comprehensive persona summaries that help AI agents develop a deeper understanding of their identity, capabilities, and role within team contexts.`;

  const mainPrompt = `Please create a comprehensive persona summary for the following AI agent:

## Agent Information
**Name:** ${agentProfile.name}
**Age:** ${agentProfile.age || "정보 없음"}세
**Gender:** ${agentProfile.gender || "정보 없음"}
**Professional Background:** ${agentProfile.professional}
**Skills & Expertise:** ${agentProfile.skills}
**Personality Traits:** ${agentProfile.personality || "정보 없음"}
**Values & Motivations:** ${agentProfile.value || "정보 없음"}
...
**Topic:** ${teamContext.topic || "정보 없음"}
${teamContext.sharedMentalModel ? `**Shared Mental Model:** ${teamContext.sharedMentalModel}` : ""}

` : ""}## Persona Summary Requirements

Create a comprehensive persona summary that helps this AI agent understand:

1. **Core Identity**: Who they are as a professional and individual
2. **Unique Strengths**: What makes them valuable and distinctive
3. **Communication Style**: How they naturally express themselves and interact
4. **Decision-Making Approach**: How their values and personality influence their choices
5. **Collaboration Patterns**: How they work best with others
6. **Innovation Approach**: How they generate ideas and solve problems
${teamContext ? "7. **Team Contribution**: How they can best contribute to this specific team and topic" : ""}

**Guidelines:**
- Write in English
- Create a cohesive narrative that integrates all aspects of their persona
- Be specific and actionable rather than generic
- Focus on how their traits work together to create their unique approach
- Help them understand their authentic self and natural working style
- Keep the summary comprehensive but readable (500-800 characters)

**Rules:**
- DO NOT simply repeat or directly quote the profile data provided above
- Instead of stating traits, describe how these characteristics manifest in their daily interactions and decision-making
- Avoid generic personality descriptions; craft insights that feel uniquely personal to this individual
- Write from a perspective of deep understanding - as if you truly know how this person thinks and operates
- Capture their complexity and subtleties without being overly dramatic or artificial
- The summary should read like genuine self-reflection, not a clinical profile analysis
- Show how their various traits interconnect and influence each other in real scenarios

Respond in the following JSON format:
{
  "personaSummary": "이 에이전트의 정체성, 강점, 소통 방식, 협업 스타일을 종합한 포괄적이고 실용적인 페르소나 설명"
}`;

  return { agentContext, mainPrompt };
};

// Team members summary generation prompt (keeping for backward compatibility)
